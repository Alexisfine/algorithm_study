...
public class _684_Redundant_Connection {
    ...
    private Set<List<Integer>> dfs(HashMap<Integer, List<Integer>> graph, HashSet<Integer> visited,
                      int prevNode, int curNode) {
                ...
                if (!visited.contains(nextNode)) {
                    ...
                    if (set != null) {
                        if (!isBack) {
                            ...
                            if (min == cycle || max == cycle) isBack = true;
                        }
                        return set;
                    }
                } else if (prevNode == nextNode) continue;
                else {
                    this.cycle = nextNode;
